      ...
      <nav className={styles.nav}>
        <div className={styles.navList}>
          {navLinks.map((navItem) => {
            const { label, pathname, external = false } = navItem; // Đặt giá trị mặc định cho external
      ...
      <Transition unmount in={menuOpen} timeout={msToNum(tokens.base.durationL)}>
        {({ visible, nodeRef }) => (
          <nav className={styles.mobileNav} data-visible={visible} ref={nodeRef}>
            {navLinks.map(({ label, pathname, external = false }, index) =>
              external ? (
                <a
                  key={label}
                  href={pathname}
                  ...
                  onClick={() => setMenuOpen(false)} // Đóng menu sau khi click
                  style={cssProps({
                ...
                <RouterLink
                  ...
                  data-visible={visible}
                  aria-current={getCurrent(pathname)}
                  onClick={handleMobileNavClick} // Xử lý click + đóng menu
                  style={cssProps({
                    transitionDelay: numToMs(
                      Number(msToNum(tokens.base.durationS)) + index * 50
                    ),
